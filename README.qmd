...
```{r}
#| fig-alt: "Event-study plot of ATT estimates by event time, with pointwise confidence intervals, for the minimum wage example."
library(did)
data(mpdta)
did_res <- pte(
  yname = "lemp",
...
What's most interesting here, is that the only "new" code that needs to be written is in [the `did_attgt` function](https://github.com/bcallaway11/ptetools/blob/master/R/attgt_functions.R).  You will see that this is a very small amount of code.
...
```{r}
#| fig-alt: "Event-study plot of ATT estimates by event time, with pointwise confidence intervals, for the COVID-19 shelter-in-place example."
...
```{r}
#| fig-alt: "Event-study plot of ATT estimates by event time, with pointwise confidence intervals, from the empirical bootstrap version of the minimum wage example."
did_res_noif <- pte(
  yname = "lemp",
  gname = "first.treat",
  tname = "year",
